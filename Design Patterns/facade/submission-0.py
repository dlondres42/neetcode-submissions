...
class DriveThruFacade:
    ...
    def takeOrder(self, orderContents: str, takeOut: bool) -> Food:
        ...
        _order = self.cashier.takeOrder(orderContents, takeOut)
        _prepared_food = self.chef.prepareFood(_order)
        _packaged_food = self.kitchenStaff.packageOrder(_prepared_food)
        return _packaged_food if takeOut else _prepared_food